import { Table, TableBody, TableCell, TableHead, TableHeaderCell, TableRow } from "@johnshandux/ledger-design-system";
import type { Transaction } from "../../../src/domain/Transaction";
import { formatCurrency, formatPostedDate, getTransactionAmounts } from "../../../src/presentation/accountDetail";
  ...
  return <>
    <div className="transactions-desktop">
      <Table ariaLabel="Transactions for this account">
        ...
        <TableBody>{transactions.map((transaction) => { const { moneyIn, moneyOut } = getTransactionAmounts(transaction); return <TableRow key={transaction.id}><TableCell className="transaction-date">{formatPostedDate(transaction.postedAt)}</TableCell><TableCell className="transaction-copy"><TransactionContext transaction={transaction} /></TableCell><TableCell><span className="transaction-status">{transaction.status}</span></TableCell><TableCell className="financial-value" align="right">{moneyOut === undefined ? "—" : formatCurrency(moneyOut, transaction.currency)}</TableCell><TableCell className="financial-value" align="right">{moneyIn === undefined ? "—" : formatCurrency(moneyIn, transaction.currency)}</TableCell><TableCell className="financial-value" align="right">{transaction.balanceAfter === undefined ? "—" : formatCurrency(transaction.balanceAfter, transaction.currency)}</TableCell></TableRow>; })}</TableBody>
      </Table>
    </div>
    <ol className="transactions-mobile" aria-label="Transactions for this account, newest first">
      {transactions.map((transaction) => { const { moneyIn, moneyOut } = getTransactionAmounts(transaction); const amount = moneyIn ?? moneyOut; return <li className="transaction-card" key={transaction.id}><div className="transaction-card-header"><div className="transaction-copy"><TransactionContext transaction={transaction} /></div><strong className="financial-value">{moneyOut !== undefined ? "−" : "+"}{formatCurrency(amount ?? 0, transaction.currency)}</strong></div><dl className="transaction-card-meta"><div><dt>Date</dt><dd>{formatPostedDate(transaction.postedAt)}</dd></div><div><dt>Status</dt><dd className="transaction-status">{transaction.status}</dd></div><div><dt>Balance</dt><dd className="financial-value">{transaction.balanceAfter === undefined ? "—" : formatCurrency(transaction.balanceAfter, transaction.currency)}</dd></div></dl></li>; })}
    </ol>
  </>;
}
